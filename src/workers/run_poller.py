...
from workers.poller_worker import PollerWorker
...
async def main():
    ...
    worker = PollerWorker(amqp_url=AMQP_URL, state_manager=client)

    await worker.connect()
    await worker.start()

    print("[poller] Running. Waiting for messages. Ctrl+C to stop.")
    try:
