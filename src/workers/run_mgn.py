...
from workers.mgn_worker import MgnWorker
...
async def main():
    ...
    worker = MgnWorker(amqp_url=AMQP_URL, state_manager=client)

    await worker.connect()
    await worker.start()

    print("[mgn-worker] Running. Waiting for messages. Ctrl+C to stop.")
    try:
